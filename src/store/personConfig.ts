...
import { defaultPersonList, prizePoolAIdList } from "./data";
...
export const usePersonConfig = defineStore("person", {
  ...
  getters: {
    ...
    // 根据当前奖项，获取对应可抽奖的人员名单
    getCurrentPrizePersonList(state: any) {
      const currentPrize = usePrizeConfig().getCurrentPrize;
      const isA = prizePoolAIdList.includes(currentPrize.id);
      
      return state.personConfig.allPersonList.filter((item: IPersonConfig) => {
        if (item.isWin) return false;
        
        // 如果是全员奖项或非A池奖项，返回所有未中奖人员
        if (currentPrize.isAll || !isA) return true;
        
        // A池奖项需要年限≥3
        const yearsOfService = Number(item.identity);
    ...
    getNotThisPrizePersonList(state: any) {
      ...
      const data = state.personConfig.allPersonList.filter(
        (item: IPersonConfig) => {
  ...
  actions: {
    ...
    moveAlreadyToNot(person: IPersonConfig) {
      ...
      const alreadyPersonListLength =
        this.personConfig.alreadyPersonList.length;
      ...
      for (let i = 0; i < alreadyPersonListLength; i++) {
        this.personConfig.alreadyPersonList =
          this.personConfig.alreadyPersonList.filter(
            (item: IPersonConfig) => item.id !== person.id
          );
      }
    },
    // 删除指定人员
    deletePerson(person: IPersonConfig) {
      if (person.id !== undefined || person.id != null) {
        this.personConfig.allPersonList =
          this.personConfig.allPersonList.filter(
            (item: IPersonConfig) => item.id !== person.id
          );
        this.personConfig.alreadyPersonList =
          this.personConfig.alreadyPersonList.filter(
